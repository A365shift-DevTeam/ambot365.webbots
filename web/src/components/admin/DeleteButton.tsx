...
import { deleteBot, deleteWebsite } from '@/lib/api';
...
export default function DeleteButton({ id, name, type, onDeleted }: DeleteButtonProps) {
  ...
  const handleDelete = async () => {
    ...
    setLoading(true);
    try {
      await (type === 'website' ? deleteWebsite(id) : deleteBot(id));
      onDeleted();
    } catch (err) {
      alert(err instanceof Error ? err.message : `An error occurred while deleting ${type}`);
    } finally {
      setLoading(false);
    }
  };
