...
export default function BotEmbed({ scriptCode, botName }: BotEmbedProps) {
  ...
  useEffect(() => {
    ...
    return () => {
      // Cleanup on unmount. In an SPA this matters more than it did under
      // Next.js: navigating between bots must not leave the previous widget
      // running, so each mount injects and each unmount removes.
